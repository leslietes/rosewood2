describe RoomsController do
  ...
  describe "sign in user" do
    ...
    context "check in" do
      ...
      it "processes a checks in" do
        expect {
          post :check_in, room_no: @room.id, occupants: [@occupant.id], start_date: Date.today
      ...
      it "adds utilities" do
        expect {
          post :check_in, room_no: @room.id, occupants: [@occupant.id], start_date: Date.today, utilities: { "#{@utility.id}" => "#{@utility.id}"}
          expect(flash[:notice]).to eq "Occupant has checked in"  
        }.to change(CheckinDetail,:count).by(1)
      end
      
      it "displays an error if room not found" do
        post :check_in, room_no: 1, occupants: [@occupant.id], start_date: Date.today
    ...
    context "occupancy list" do
      ...
      it "displays vacant room" 
      
      it "displays occupied room and occupant" 
    end
